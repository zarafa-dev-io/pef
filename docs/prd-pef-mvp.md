...
- **QO-8** — ~~Accès au code pour la rétro-doc~~ **Tranchée (v0.14.0, DEC-006)** : workspace VS Code multi-root (repo PEF + repos de code) ; `codeRefs` au format `<repo>/<chemin>[:<lignes>]`, non validés par le CLI — leur péremption est détectée par `RefreshDocumentation`, pas garantie par construction.
